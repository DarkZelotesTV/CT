...
export const ServerSettingsModal = ({ serverId, onClose }: ServerSettingsProps) => {
  ...
  const tabs = [
    ...
    { key: 'channels', label: 'Kanäle & Passwörter' },
    { key: 'roles', label: 'Rollen & Rechte' },
    ...
    <div className="fixed inset-0 bg-black/80 z-[100] flex items-center justify-center animate-in fade-in">
      <div className="bg-dark-100 w-[980px] h-[650px] rounded-lg shadow-2xl flex overflow-hidden border border-dark-400">
        <div className="w-64 bg-dark-200 p-4 flex flex-col">
          ...
          {tabs.map((tab) => (
            <div
              ...
              className={`px-2 py-1.5 rounded cursor-pointer mb-1 ${
                activeTab === tab.key ? 'bg-dark-300 text-white' : 'text-gray-400 hover:bg-dark-300'
              }`}
            >
              {tab.label}
            </div>
          ...
          <div className="mt-auto pt-4 border-t border-dark-400">
            <div
              ...
              className="text-red-400 px-2 py-1.5 rounded cursor-pointer hover:bg-red-500/10 flex items-center gap-2"
            >
        ...
        <div className="flex-1 bg-dark-100 p-6 md:p-8 relative overflow-hidden flex flex-col">
          ...
          <div className="sticky top-0 z-10 -mx-6 md:-mx-8 px-6 md:px-8 pb-3 bg-dark-100/95 backdrop-blur border-b border-dark-400">
                ...
                <button
                  ...
                  className={`px-3 py-1.5 rounded-full text-sm whitespace-nowrap border transition-colors ${
                    activeTab === tab.key
                      ...
                      : 'bg-dark-300 text-gray-300 border-dark-400 hover:text-white'
                  }`}
                >
                  {tab.label}
                </button>
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar pr-1">
              <div className="grid lg:grid-cols-[1.2fr_1fr] gap-6">
                <div className="space-y-6">
                  ...
                  <div className="bg-dark-200 border border-dark-400 rounded-lg p-4 space-y-4">
                    <h3 className="text-gray-300 uppercase text-xs font-bold">Allgemein</h3>
                    <div className="grid md:grid-cols-2 gap-4">
                      <label className="space-y-2">
                        <span className="block text-xs font-bold text-gray-400 uppercase">Servername</span>
                        <input
                          type="text"
                          value={serverName}
                      ...
                      <div className="flex items-end">
                        <button
                          ...
                          className="w-full md:w-auto bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded font-medium flex items-center gap-2 justify-center"
                        >
                ...
                <div className="bg-dark-200 border border-dark-400 rounded-lg p-4 space-y-3">
                  <h3 className="text-gray-300 uppercase text-xs font-bold">Hinweise</h3>
                  <p className="text-sm text-gray-400">
                    Nutze die Tabs oben, um schnell zwischen Mitgliedern, Kanälen und Rollen zu wechseln. Alle Formularfelder sind responsive und passen sich an dein Layout an.
                  </p>
                </div>
              </div>
            </div>
          ...
          {activeTab === 'members' && (
            <div className="flex-1 flex flex-col min-h-0">
              <div className="sticky top-[62px] -mx-6 md:-mx-8 px-6 md:px-8 pt-4 pb-3 bg-dark-100/95 backdrop-blur border-b border-dark-400 flex items-center justify-between z-10">
                <div className="flex flex-col gap-1">
                  <h1 className="text-2xl font-bold text-white">Mitglieder ({members.length})</h1>
                  <p className="text-gray-400 text-sm">Verwalte Rollen und entferne Mitglieder bei Bedarf.</p>
                </div>
                <button
                  onClick={loadMembers}
                  className="text-xs bg-dark-300 px-3 py-1 rounded text-gray-300 hover:text-white flex items-center gap-2 border border-dark-400"
                >
                  <Loader2 size={14} className="animate-spin" /> Refresh
                </button>
              </div>
              <div className="flex-1 overflow-y-auto custom-scrollbar space-y-3 pr-2 pt-4">
                {loadingMembers ? (
                      ...
                      <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                        ...
                        <div className="flex flex-col gap-2 md:flex-row md:items-center md:gap-3 w-full md:w-auto">
                          <select
                            ...
                            className="bg-dark-300 text-white text-xs rounded px-2 py-1 border border-dark-400 min-w-[160px] flex-1"
                          >
                            {roles.map((r) => (
                          ...
                          <button
                            ...
                            className="text-red-400 hover:text-red-300 text-xs flex items-center gap-1 justify-center border border-red-500/40 rounded px-2 py-1"
                          >
                            <Trash2 size={14} /> Kick
                          </button>
                        </div>
          ...
          {activeTab === 'channels' && (
            <div className="flex-1 flex flex-col min-h-0">
              <div className="sticky top-[62px] -mx-6 md:-mx-8 px-6 md:px-8 pt-4 pb-3 bg-dark-100/95 backdrop-blur border-b border-dark-400 flex flex-col gap-2 md:flex-row md:items-center md:justify-between z-10">
                <div>
                  <h1 className="text-2xl font-bold text-white">Kanäle</h1>
                  <p className="text-gray-400 text-sm">Struktur und Passwörter verwalten</p>
                </div>
                <div className="flex flex-col gap-2 md:flex-row md:items-center">
                  ...
                  <button
                    ...
                    className="bg-primary px-3 py-1 rounded text-white flex items-center gap-2 text-sm justify-center"
                  >
              ...
              <div className="flex-1 overflow-y-auto custom-scrollbar pr-2 pt-4 space-y-4">
                <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    <div className="flex items-center justify-between">
                      <h3 className="text-gray-400 text-xs uppercase font-bold">Ohne Kategorie</h3>
                      <span className="text-[11px] text-gray-500">{structure.uncategorized.length} Kanäle</span>
                    </div>
                      ...
                      <div key={c.id} className="bg-dark-200 border border-dark-400 rounded p-3 space-y-2">
                        <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
                          <input
                            ...
                            className="bg-dark-300 text-white px-2 py-1 rounded text-sm border border-dark-400 flex-1"
                          />
                          <div className="flex items-center gap-1 self-start md:self-auto">
                            <button
                              onClick={() => moveChannel(structure.uncategorized, idx, -1, null)}
                            ...
                            <button
                              onClick={() => setSelectedChannelForOverrides(c.id)}
                              className="p-1 text-gray-400 hover:text-white"
                              title="Overrides"
                            >
                              <Settings2 size={14} />
                    ...
                    {structure.categories.map((cat) => (
                      <div key={cat.id} className="bg-dark-200 border border-dark-400 rounded p-3 space-y-2">
                        <div className="flex items-center justify-between">
                          ...
                          <span className="text-[11px] text-gray-500">{cat.channels.length} Kanäle</span>
                        </div>
                        {cat.channels.map((c, idx) => (
                          <div key={c.id} className="bg-dark-300/60 border border-dark-400 rounded p-3 space-y-2">
                            <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
                              <input
                                ...
                                onChange={(e) =>
                                  setStructure((prev) => ({
                                    ...prev,
                                    categories: prev.categories.map((cg) =>
                                      cg.id === cat.id
                                        ? { ...cg, channels: cg.channels.map((ch) => (ch.id === c.id ? { ...ch, name: e.target.value } : ch)) }
                                        : cg
                                    ),
                              ...
                              />
                              <div className="flex items-center gap-1 self-start md:self-auto">
                                <button onClick={() => moveChannel(cat.channels, idx, -1, cat.id)} className="p-1 text-gray-400 hover:text-white">
                                  <ArrowUp size={14} />
                                </button>
                                <button onClick={() => moveChannel(cat.channels, idx, 1, cat.id)} className="p-1 text-gray-400 hover:text-white">
                                  <ArrowDown size={14} />
                                </button>
                                <button onClick={() => setSelectedChannelForOverrides(c.id)} className="p-1 text-gray-400 hover:text-white" title="Overrides">
                                  <Settings2 size={14} />
                                </button>
                              </div>
                            </div>
                                ...
                                <select
                                  ...
                                  onChange={(e) =>
                                    setStructure((prev) => ({
                                      ...
                                      categories: prev.categories.map((cg) =>
                                        cg.id === cat.id
                                          ? { ...cg, channels: cg.channels.map((ch) => (ch.id === c.id ? { ...ch, type: e.target.value as any } : ch)) }
                                          : cg
                                      ),
                                      uncategorized: prev.uncategorized,
                                    }))
                                ...
                                <input
                                  ...
                                  onChange={(e) =>
                                    setStructure((prev) => ({
                                      ...
                                      categories: prev.categories.map((cg) =>
                                        cg.id === cat.id
                                          ? { ...cg, channels: cg.channels.map((ch) => (ch.id === c.id ? { ...ch, default_password: e.target.value } : ch)) }
                                          : cg
                                      ),
                                      uncategorized: prev.uncategorized,
                                    }))
                                ...
                                <input
                                  ...
                                  onChange={(e) =>
                                    setStructure((prev) => ({
                                      ...
                                      categories: prev.categories.map((cg) =>
                                        cg.id === cat.id
                                          ? { ...cg, channels: cg.channels.map((ch) => (ch.id === c.id ? { ...ch, join_password: e.target.value } : ch)) }
                                          : cg
                                      ),
                                      uncategorized: prev.uncategorized,
                                    }))
                            ...
                            <div className="flex justify-end">
                              <button
                                onClick={() => saveChannel({ ...c, category_id: cat.id })}
                                className="bg-green-600 text-white px-3 py-1 rounded text-sm flex items-center gap-1"
                              >
                                <Save size={14} /> Speichern
                              </button>
                ...
                {selectedChannelForOverrides && (
                  <div className="mt-4 border-t border-dark-400 pt-4 space-y-3">
                    <div className="flex items-center gap-2">
                      <Settings2 size={16} className="text-gray-400" />
                      <span className="text-gray-200 text-sm">Overrides für Kanal #{selectedChannelForOverrides}</span>
                    </div>
                    <div className="grid md:grid-cols-[1fr_auto] gap-2 md:items-center">
                      <select
                      ...
                      <button
                        ...
                        className="bg-blue-600 text-white px-3 py-1 rounded text-sm flex items-center gap-1 justify-center"
                      >
                        <Save size={14} /> Speichern
                      </button>
                    </div>
            ...
            <div className="flex-1 flex flex-col min-h-0 gap-4">
              <div className="sticky top-[62px] -mx-6 md:-mx-8 px-6 md:px-8 pt-4 pb-3 bg-dark-100/95 backdrop-blur border-b border-dark-400 flex flex-col gap-2 md:flex-row md:items-center md:justify-between z-10">
                <div>
                  ...
                  <p className="text-gray-400 text-sm">Aktiviere Rechte und verwalte die Reihenfolge der Rollen.</p>
                </div>
                <button
                  onClick={createRole}
                  className="bg-primary px-3 py-1 rounded text-white flex items-center gap-2 text-sm justify-center"
                      ...
                      <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
                        <div className="flex items-center gap-2">
                          ...
                          <input
                            ...
                            className="bg-dark-300 text-white text-sm px-2 py-1 rounded border border-dark-400 flex-1"
                          />
                        </div>
                        {!role.is_default && (
                          <button onClick={() => deleteRole(role.id)} className="text-red-400 hover:text-red-300 text-xs self-start md:self-auto">
                            Löschen
                          </button>
                        )}
                      </div>
