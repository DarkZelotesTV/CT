    ...
    <div className="flex-1 flex flex-col h-full bg-gradient-to-br from-[#0b0c10] via-[#0d0f15] to-[#0b0c10] relative font-sans overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar w-full">
        <div className="min-h-full flex items-center justify-center p-4 md:p-8">
          <div className="w-full max-w-6xl grid grid-cols-1 lg:grid-cols-[1.6fr_1fr] gap-6 relative z-10">
            ...
            <div className="bg-[#0f1117]/90 border border-white/10 rounded-3xl shadow-2xl p-5 md:p-8 backdrop-blur-xl flex flex-col gap-6 animate-in slide-in-from-bottom-4 duration-500">
              ...
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-white/5 pb-4 md:pb-6">
                <div>
                  ...
                  <div className="text-2xl font-bold text-white flex items-center gap-2 flex-wrap">
                    <span className="truncate max-w-[200px] md:max-w-xs">{channel.name}</span>
                    <ChevronRight className="text-gray-600 hidden md:block" size={20} />
                    <span className="text-sm font-medium text-gray-400 hidden md:block">Eingangshalle</span>
                  </div>
                  <div className="flex items-center gap-2 mt-2 text-xs font-medium">
                    ...
                    <span className="text-gray-400">{connectionLabel}</span>
                  </div>
                </div>
                ...
                <button
                  ...
                  className="flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-white/5 hover:bg-white/10 text-xs font-semibold text-gray-300 border border-white/10 transition-all hover:border-white/20 active:scale-95 w-full md:w-auto"
                >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className={`p-4 rounded-2xl border transition-all duration-300 ${micMuted ? 'bg-red-500/5 border-red-500/20' : 'bg-white/5 border-white/10'}`}>
                   <div className="flex items-center justify-between mb-3">
                      ...
                      <button
                        ...
                        className={`px-3 py-1.5 rounded-lg text-[10px] uppercase font-bold tracking-wide border transition-all ${micMuted ? 'bg-red-500/20 border-red-500/50 text-red-200' : 'bg-green-500/10 border-green-500/30 text-green-300 hover:bg-green-500/20'}`}
                      >
                        {micMuted ? 'Stumm' : 'Live'}
                      </button>
                   </div>
                   <div className="relative">
                      <select
                        ...
                        className="w-full bg-black/40 border border-white/10 rounded-xl px-3 py-2.5 text-xs text-gray-200 focus:outline-none focus:border-cyan-500/50 focus:ring-1 focus:ring-cyan-500/20 appearance-none transition-colors"
                      >
                ...
                <div className={`p-4 rounded-2xl border transition-all duration-300 ${muted ? 'bg-red-500/5 border-red-500/20' : 'bg-white/5 border-white/10'}`}>
                   <div className="flex items-center justify-between mb-3">
                      ...
                      <button
                        ...
                        className={`px-3 py-1.5 rounded-lg text-[10px] uppercase font-bold tracking-wide border transition-all ${muted ? 'bg-red-500/20 border-red-500/50 text-red-200' : 'bg-white/10 border-white/10 text-gray-300 hover:bg-white/15'}`}
                      >
                        {muted ? 'Stumm' : 'Hörbar'}
                      </button>
                   </div>
                ...
                <div className="p-4 rounded-2xl bg-white/5 border border-white/10 hover:border-white/20 transition-colors">
                    <div className="flex items-center justify-between mb-3">
                      ...
                      <span className="text-[10px] uppercase text-gray-600 font-bold bg-black/20 px-2 py-1 rounded">Optional</span>
                    </div>
                    <div className="relative mb-2">
                      <select
                        value={selectedVideoInputId || ''}
                        onChange={(e) => handleDeviceChange('videoinput', e.target.value)}
                        className="w-full bg-black/40 border border-white/10 rounded-xl px-3 py-2.5 text-xs text-gray-200 focus:outline-none focus:border-purple-500/50 focus:ring-1 focus:ring-purple-500/20 appearance-none transition-colors"
                      >
              ...
              {(error || localError) && (
                <div className="p-3 bg-red-500/10 border border-red-500/20 rounded-xl flex items-center gap-3 animate-in fade-in slide-in-from-top-2">
                    <div className="p-1.5 bg-red-500/20 rounded-full text-red-400"><MicOff size={14}/></div>
            ...
            <div className="bg-[#0f1117]/90 border border-white/10 rounded-3xl shadow-2xl p-6 md:p-8 flex flex-col gap-6 backdrop-blur-xl h-fit lg:sticky lg:top-8 animate-in slide-in-from-bottom-8 duration-700">
               ...
               <div className="flex items-center gap-4">
                  <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center text-white font-bold text-xl shadow-lg shadow-indigo-500/20">
