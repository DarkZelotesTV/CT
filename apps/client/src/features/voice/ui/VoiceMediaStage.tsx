...
export const VoiceMediaStage = ({ layout }: { layout: 'grid' | 'speaker' }) => {
  ...
  useEffect(() => {
    ...
    activeRoom.on(RoomEvent.ActiveSpeakersChanged, (s) => setActiveSpeakerSid(s[0]?.sid ?? null));
    return () => { activeRoom.off(RoomEvent.ParticipantConnected, bump); activeRoom.off(RoomEvent.ParticipantDisconnected, bump); };
  }, [activeRoom]);
